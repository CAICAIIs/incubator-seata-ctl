...
func newShowCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "show",
		Short: "Show one global transaction",
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if xid == "" {
				return fmt.Errorf("xid is required")
			}

			session, err := seata.QueryGlobalSessionByXID(xid)
			if err != nil {
				return err
			}
			result, err := seata.FormatGlobalSessionDetail(session, output)
			if err != nil {
				return err
			}
			fmt.Println(result)
	...
	cmd.SetUsageTemplate(common.GetUsageTmpl("transaction show"))
